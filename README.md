...
- 背景支持随机图片轮播；未配置自定义图片时，默认会拉取 Bing 最近壁纸并随机轮播，失败时回退到内置 SVG 背景组
...
| `BACKGROUND_BING_MARKET` | Variable | 否 | 默认 Bing 壁纸市场，如 `zh-CN`、`en-US`，默认 `zh-CN` |

说明：

- `BACKGROUND_IMAGE_URLS` 不配也能用，页面会优先拉取 Bing 最近壁纸，失败后退回到内置背景图
